...
import { MessageSquare, Mail, Send, Paperclip, Clock, CheckCircle2, AlertCircle, FileText, Users, Sparkles, ExternalLink, Loader2 } from "lucide-react";
import { sendCampaign, uploadCampaignAttachment, fetchCampaigns } from "@/lib/api";
...
export function MessagingCenter() {
  ...
  const [recipientType, setRecipientType] = useState<"ALL" | "STATUS" | "TRAINING">("ALL");
        ...
        <div className="lg:col-span-2 glass-card p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
          ...
          <form onSubmit={handleSendCampaign} className="space-y-5 pt-2 border-t border-slate-800">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs font-bold text-slate-300 block mb-1.5">Destinataires *</label>
                <select
                  value={recipientType}
                ...
                >
                  ...
                  <option value="TRAINING">Filtrer par Formule</option>
                </select>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <input
                ...
                className="w-4 h-4 rounded border-slate-700 bg-slate-900 text-brand-cyan focus:ring-0"
              />
            ...
            <button
              type="submit"
              disabled={isSending}
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-brand-500 via-blue-600 to-brand-cyan text-white font-extrabold text-sm shadow-lg shadow-brand-500/30 hover:shadow-brand-500/50 hover:scale-[1.01] active:scale-[0.99] transition-all flex items-center justify-center gap-2"
            >
              {isSending ? (
                <>
                ...
                <>
                  ...
                  <span>{isScheduled ? "⏰ Programmer le Rappel Automatique" : "🚀 Lancer la Campagne & Générer les Envois"}</span>
                </>
              )}
            </button>
          </form>

          {/* Generated WhatsApp Dispatcher Links Modal/Result Section */}
          {sentResult && sentResult.recipients && (
            <div className="mt-8 p-6 rounded-2xl bg-slate-900 border border-emerald-500/40 space-y-4 shadow-xl">
              <div className="flex items-center gap-2 text-emerald-400 font-extrabold text-sm">
                ...
                <span>Campagne générée avec succès ({sentResult.recipients.length} candidats ciblés) !</span>
              </div>

              <p className="text-xs text-slate-300">
                Vous pouvez cliquer directement sur le bouton WhatsApp de chaque candidat ci-dessous pour ouvrir l'application avec le message et les variables déjà personnalisés :
              </p>
                ...
                {sentResult.recipients.map((rec: any) => (
                  <div key={rec.id} className="p-3 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between gap-3">
                    <div>
                      ...
                      <span className="text-[10px] text-slate-400 font-mono">{rec.phone} • {rec.email}</span>
                    </div>
